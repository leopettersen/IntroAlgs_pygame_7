...
class Meteoro:
    ...
    def mover(self, screen, dt, pontos_atual):
        ...
        if self.explodindo:
            ...
            if self.frame_explosao >= len(Meteoro.frames_explosao):
                self.explodindo = False
                self.frame_explosao = 0
                self.redefinir_posicao(screen)

        elif self.rect.y <= screen.height:
    ...
    def redefinir_posicao(self, screen):
        ...
        self.rect.x = random.randint(
            30,
            screen.width - self.rect.width - 30
        )

    def desenhar(self, screen):
        if self.explodindo:
            screen.blit(
                Meteoro.frames_explosao[self.frame_explosao],
